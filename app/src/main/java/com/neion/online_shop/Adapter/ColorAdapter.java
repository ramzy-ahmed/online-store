...
import com.neion.online_shop.databinding.ViewholderColorBinding;
...
public class ColorAdapter extends RecyclerView.Adapter<ColorAdapter.Viewholder> {
    ...
    public void onBindViewHolder(@NonNull ColorAdapter.Viewholder holder, int position) {
        ...
        if(selectedPosition == holder.getAdapterPosition()){
            ...
            Glide.with(context).load(unwrappedDrawable).apply(RequestOptions.centerCropTransform()).into(holder.binding.colorLayout);
        }else{
            Drawable unwrappedDrawable= AppCompatResources.getDrawable(context,R.drawable.color_selected);
            Drawable wrappedDrawable = DrawableCompat.wrap(unwrappedDrawable);
            DrawableCompat.setTint(wrappedDrawable, Color.parseColor(items.get(position)));
            Glide.with(context).load(wrappedDrawable).apply(RequestOptions.centerCropTransform()).into(holder.binding.colorLayout);
